...
int Plot_Final_datapf_JEC(std::string filename){
  ...
  JetCorrectorParameters *L2JetPar=new JetCorrectorParameters("/afs/cern.ch/user/l/lcunquei/TestTrigger23/ParallelMC_L2Relative_AK4PF_offline.txt");  
  ...
  TFile *input = TFile::Open(filename.data());
    ...
    TTreeReaderArray<Float_t> jtPfNHF(emuReader,"jtPfNHF");
	...
	double Correction = JEC->getCorrection();
		double CorrectedPT = jtpt[l]*JEC->getCorrection();
