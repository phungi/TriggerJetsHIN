...
int Plot_Final_pp_data_pflow_JEC(std::string filename){
  ...
  while (emuReader.Next()) {
    ...
    for(long unsigned n=0;n<(*emuJetPt).size();n++){
      ...
      if(dr<drmin){ //effective cutoff distance of 10
        drmin=dr;
        l1pt=(*emuJetPt)[n];
        l1eta=(*emuJetEta)[n];
        l1phi=(*emuJetPhi)[n];
    ...
    //cutoff distance introduced here
    if(drmin>0.4) continue;
    if(l1pt==0) continue;
    // std::cout << "should be filling calomax histogram" << std::endl;
    //now we have an L1 jet matched to the highest p_T (corrected) calo jet
