...
int Data_pp_pflow_JEC_calo_first(std::string filename){
  TH1::SetDefaultSumw2( );
  //int bitmin=274;
  //int bitmax=281;
  //int binnum=7;
  ...
  hjetl1pfdR=new TH1F(Form("jetcalopf"),Form("jetcalopf"),150,0,3.16);
  ...
  while (emuReader.Next()) {
    ...
    double PF_match_pt= -std::numeric_limits<double>::max();
    double PF_match_eta=-std::numeric_limits<double>::max();
    double PF_match_phi=-std::numeric_limits<double>::max();
    ...
    double l1pt=-std::numeric_limits<double>::max();
    double l1eta=-std::numeric_limits<double>::max();
    double l1phi=-std::numeric_limits<double>::max();
    double drmin=std::numeric_limits<double>::max();
    for(long unsigned n=0;n<(*emuJetPt).size();n++){
      if(emuJetPt->at(n) > l1pt){
        l1pt=emuJetPt->at(n);
        l1eta=emuJetEta->at(n);
        l1phi=(*emuJetPhi)[n];
      }
    }
    if(l1pt==-std::numeric_limits<double>::max()) continue;
    for(int l=0;l<*nref;l++){
      ...
      // std::cout << "Setting jet variables to: " << jtpt[l] << " " << jteta[l] << " " << jtphi[l] << std::endl;
      JEC->setJetPt(20);
      JEC->setJetEta(-1);
      ...
      // std::cout << Correction << "after jet correction" << std::endl;
      if(jteta[l]>2 || jteta[l]<-2) continue;
      double dr=getDR(l1eta,l1phi,jteta[l],jtphi[l]); 
      if(dr < drmin){
        PF_match_pt=CorrectedPT;
        PF_match_eta=jteta[l];
        PF_match_phi=jtphi[l];
        drmin = dr;
      }
    }
    ...
    hjet_all_calo->Fill(PF_match_pt);
    if(drmin > 0.4) continue;
    hjet_all_calo_matched->Fill(PF_match_pt);
    for(int l=0;l<binnum;l++){
      if(l1pt>limits[l]){ 
        hjet_fired_calo[l]->Fill(PF_match_pt);
      }
    }
  }
  delete jets;
